	...
	const handleSubmit = useCallback( async () => {
		...
		try {
			...
			if (response.status === 201) {
				const eventId = response.data.id || response.data.eventId;
				
				// 2단계: 이미지 업로드 (이미지가 있는 경우)
				if (thumbnailImage && eventId) {
					const formData = new FormData();
					formData.append('image', thumbnailImage);

					try {
						await apiClient.post(`/event/upload/${eventId}`, formData, {
							headers: {
						...
						alert('이벤트는 생성되었지만 이미지 업로드에 실패했습니다.');
					}
				}
		...
		<>
			<div className="flex flex-col items-center w-full min-h-screen p-4 sm:p-8 bg-gray-50 font-sans">
				...
				<div className="flex justify-center items-center w-full max-w-6xl relative py-8 border-b-2 border-[#e6e6e6]"
					...
					<p className="text-3xl md:text-4xl font-medium text-[#333]">이벤트</p>
				</div>
