...
const EventsListPage = () => {
		...
		const fetchEvents = async () => {
			...
			try {
				...
				if (searchParams.keyword || searchParams.startDate || searchParams.endDate) {
					...
					response = await apiClient.get(`/event/search?${queryString}`);
				} else {
					response = await apiClient.get(`/event?page=${currentPage}`);
				}

				setEvents(response.data.events || response.data.content || []);
				setCurrentPage(response.data.currentPage || response.data.number || 0);
	...
	// 삭제 버튼 클릭 핸들러 (모달 열기)
	const handleDeleteClick = (eventId, event) => {
	...
	const handleDeleteConfirm = async () => {
		try {
			await apiClient.delete(`/event/${deleteTargetId}`);
	...
	const handleEdit = (eventId, event) => {
		event.preventDefault();
		event.stopPropagation();
		navigate(`/bodyswitch-admin/event/edit/${eventId}`);
	};

	// 게시글 작성 페이지로 이동
	const handleCreateEvent = () => {
		navigate("/bodyswitch-admin/event/create");
	};
			...
			<div className="flex flex-col items-center max-w-[1400px] mx-auto w-full">
						...
						<div className="flex-1 w-full min-w-[300px]">
							...
							<div className="flex items-center gap-2">
								<input
									...
									placeholder="제목 또는 담당자명을 입력해 주세요"
				...
				<div className="flex flex-col justify-start items-start w-full">
					...
					<div className="flex justify-start items-center w-full border border-[#b3b3b3] bg-gray-50">
						...
						<div className="flex justify-center items-center w-[20%] min-w-[150px] h-12 py-2.5">
							<p className="text-base font-semibold text-neutral-800">제목</p>
						</div>
						<div className="flex justify-center items-center w-[15%] min-w-[120px] h-12 py-2.5">
							<p className="text-base font-semibold text-neutral-800">부제목</p>
						</div>
						<div className="flex justify-center items-center w-[8%] min-w-[80px] h-12 py-2.5">
							<p className="text-base font-semibold text-neutral-800">작성자</p>
						</div>
						<div className="flex justify-center items-center w-[25%] min-w-[200px] h-12 py-2.5">
							<div className="flex items-center gap-2">
								<p className="text-base font-semibold text-neutral-800">시작일</p>
								<p className="text-base font-semibold text-neutral-800">~</p>
								<p className="text-base font-semibold text-neutral-800">종료일</p>
							</div>
						</div>
						...
						<div className="flex justify-center items-center w-[16%] min-w-[120px] h-12 py-2.5">
					...
					{events && events.length > 0 ? (
						events.map((event) => (
							<div
								key={event.id}
								onClick={() => navigate(`/bodyswitch-admin/event/${event.id}`)}
							...
							>
								<div className="flex justify-center items-center w-[8%] min-w-[60px] py-3">
									<p className="text-base font-medium text-neutral-800">{event.id}</p>
								</div>
								<div className="flex justify-start items-center w-[20%] min-w-[150px] px-4 py-3">
									<p className="w-full text-base font-medium text-neutral-800 truncate">
										{event.title}
									</p>
								</div>
								<div className="flex justify-start items-center w-[15%] min-w-[120px] px-4 py-3">
									<p className="w-full text-base font-medium text-neutral-800 truncate">
										{stripHtmlTags(event.subTitle)}
									</p>
								</div>
								<div className="flex justify-center items-center w-[8%] min-w-[80px] py-3">
									<p className="text-base font-medium text-neutral-800">{event.author}</p>
								</div>
								<div className="flex justify-center items-center w-[25%] min-w-[200px] py-3">
									<div className="flex items-center gap-2">
								...
								<div className="flex justify-center items-center w-[8%] min-w-[70px] py-3">
									<p className="text-base font-medium text-neutral-800">{event.viewCount || 0}</p>
								</div>
								<div
									onClick={(e) => e.stopPropagation()}
									className="flex justify-center items-center w-[16%] min-w-[120px] py-3 gap-1"
								>
									<button
